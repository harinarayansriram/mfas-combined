...
#include "hashorva/solution_instance.h"
...
void random_toposort(SolutionInstance* instance, const Connectome* connectome, int verbosity) {
    ...
    if (verbosity >= 2) {
        printf("    Thread %d: Performing random toposort...\n", omp_get_thread_num());
    }
...
// This carefully mirrors the logic from the Go `computeChange` function.
...
void run_simanneal_parallel_with_toposhuffle(
    ...
    #pragma omp parallel
    {
        ...
        SolutionInstance* thread_instance = create_solution_instance(connectome, instance->solution, false);
        if (!thread_instance) {
             fprintf(stderr, "Error: Thread %d failed to allocate SolutionInstance.\n", thread_id);
             // This will likely cause issues, OpenMP might abort or hang. Need better error handling.
             #pragma omp cancellation point parallel
        }
        // Deep copy initial state from the shared starting 'instance'
        //memcpy(thread_instance->solution, instance->solution, n * sizeof(int));
        memcpy(thread_instance->node_to_position, instance->node_to_position, n * sizeof(int));
        thread_instance->forward_score = instance->forward_score;
        thread_instance->solution_size = n;


        long long thread_current_score = thread_instance->forward_score;
        ...
        memcpy(thread_best_solution, thread_instance->solution, n * sizeof(int));
        ...
        for (long long iter = 1; iter <= iterations_per_thread; ++iter) {
            ...
            dense_idx_a = thread_instance->solution[pos_a];
            ...
            if (total_deg == 0) {
                 ...
                 pos_b = thread_instance->node_to_position[dense_idx_b];
            }
            dense_idx_b = thread_instance->solution[pos_b]; // Get b's dense index
            ...
            if (pos_a == pos_b) {
                ...
                dense_idx_a = thread_instance->solution[pos_a];
                dense_idx_b = thread_instance->solution[pos_b];
            }


            // --- Calculate Score Change ---
            long long delta_score = calculate_delta_score_on_swap_positions(thread_instance, connectome, dense_idx_a, dense_idx_b);
            trials++;
            ...
            if (accepted) {
                ...
                thread_instance->solution[pos_a] = dense_idx_b;
                thread_instance->solution[pos_b] = dense_idx_a;
                thread_instance->node_to_position[dense_idx_a] = pos_b;
                thread_instance->node_to_position[dense_idx_b] = pos_a;

                // Update the score
                thread_current_score += delta_score;
                thread_instance->forward_score = thread_current_score; // Keep instance score consistent
            ...
            if (go_back_to_best_window > 0 && iter % go_back_to_best_window == 0) {
                 if (verbosity >= 10) {
                     printf("    Thread %d: Iter %lld, reverting to best state (Score: %lld)\n", thread_id, iter, thread_best_score);
                 }
                 memcpy(thread_instance->solution, thread_best_solution, n * sizeof(int));
                 // Rebuild node_to_position map
                 for (int i = 0; i < n; ++i) {
                     thread_instance->node_to_position[thread_instance->solution[i]] = i;
                 }
                 thread_current_score = thread_best_score;
                 thread_instance->forward_score = thread_best_score;
            }

            // --- Perform Random Toposort Periodically ---
             if (toposhuffle_frequency > 0 && iter % toposhuffle_frequency == 0) {
                 random_toposort(thread_instance, connectome, verbosity);
                 // Toposort updates instance score, so update current score
                 thread_current_score = thread_instance->forward_score;
             ...
             if (verbosity == 10 && update_wavelength > 0 && iter % update_wavelength == 0) {
                  ...
                  printf("Thr %d: Step %lld/%lld | T: %.6f | Score: %lld | Best: %lld | Accept: %.2f%% | Improve: %.2f%%\n",
                         thread_id, iter, iterations_per_thread, t, thread_current_score, thread_best_score, accept_rate, improve_rate);
        ...
        #pragma omp critical
        {
             if (thread_best_score > global_best->best_score) {
                 if (verbosity >= 1) {
                     printf("Thread %d found new global best score: %lld (was %lld)\n",
                            thread_id, thread_best_score, global_best->best_score);
                 }
